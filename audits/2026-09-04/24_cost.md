...
**DISPOSITION UPDATE (2026-09-04, same session).** **Finding 2 (parallelism) -- IMPLEMENTED, still not claimed.** `jobs` is a parameter, defaulting to 1, with one isolated tree copy per worker: sharing a sandbox and mutating one file from several threads would produce exactly the class of result this module exists to prevent -- a verdict about a file whose contents were something else by the time the tests read it. The reported 3.5x is NOT reproduced and NOT claimed; it was measured under a load its own author documented. What ships instead is the property that can be verified on a noisy machine and is the only one a harness may not trade for speed: `jobs=4` reaches the same mutants-run count, the same kill count and the same survivor keys as `jobs=1`, and survivors come back in source order rather than completion order. It stays off by default because the remaining risk is in the CONSUMER -- a suite that binds a port or shares a database gets false kills from concurrency, and only its owner can rule that out.

**Finding 4 (parallel survivor confirmations) -- RESOLVED as a consequence.** Each partition confirms its own survivors in its own sandbox, so confirmations now run concurrently whenever `jobs > 1` without any separate machinery.
